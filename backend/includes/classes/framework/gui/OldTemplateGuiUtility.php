...
/**
 * Description of OldTemplateGuiUtility
 *
 * @author suyash
 */
class OldTemplateGuiUtility
{
    ...
    protected function initialiseJs()
    {
        ...
        $this->defaultJsFiles[count($this->defaultJsFiles)] = array("file" => "scriptaculous",
...
//        $this->javascriptImages[count($this->javascriptImages)] = array("image" => "collapse.png", "var" => "collapse");//example
    ...
    public function getLoginDisplay($title, $data)
    {
        $output = "";
        ...
        $output .= "<body>";
        $output .= "<div id='main_container'>$data</div>";
    ...
    public function getNormalDisplay($title, $leftContentData, $contentData, $navigation = "",
                                     $loadFacebookLibrary = false)
    {
        $output = "";
...
//        $output .= $this->loadGoogleAnalytics();
        ...
        {
            $output .= OldTemplateGuiUtility::loadFacebookLibrary();
        }

        $output .= "<div id='main_container'>";
        $output .= OldTemplateGuiUtility::getHeader($navigation);
        $output .= OldTemplateGuiUtility::getMainContent($leftContentData, $contentData);
        $output .= OldTemplateGuiUtility::getFooter();
        $output .= "</div>";
    ...
    private static function getHeader($navigation)
    {
        ...
        $output
            .=
            "<img src='".UrlConfiguration::getImageSrc("supercash.jpg", "template")."' alt='' title='' border='0' />";
        $output .= "</a></div>";
    ...
    private static function getMainContent($leftContentData, $contentData)
    {
        ...
        $output .= "<h2>Welcome to Supercash Administration</h2>";
